"""Jev injection guard for OpenAI Codex CLI: judge content before it loads.

Codex PreToolUse hook. Codex has no dedicated file-read or web-fetch tool:
fetches happen through shell commands and reads through MCP tools or the
shell, so this hook guards what is visible before execution:

  Bash commands   extract http(s) URLs from the command line (curl, wget,
                  Invoke-WebRequest...) and judge each target BEFORE the
                  command runs. No URL -> nothing to judge, allow.
  mcp__* tools    judge the first few arguments that look like a URL or an
                  existing file path.
  anything else   allow (nothing loadable is inspectable at PreToolUse).

Same engine as the ZCode port: three Noul questions to Jev in one call
(agent_directive, harmful_intent, concealed_directive); deny if ANY signal
>= threshold. Guard errors fail OPEN by default (JEV_GUARD_FAIL_MODE=block
for fail-closed). Set TYPESAFE_API_KEY before use.

Exit codes: 0 = allow, 2 = block (Codex accepts exit 2 + stderr reason for
PreToolUse deny), 1 = guard error (manual mode). Hook mode keeps stdout
empty.

Usage:
  guard.py --hook                    # Codex PreToolUse hook (JSON on stdin)
...
JEV_GUARD_MAX_CHARS (60000 chunk size), JEV_GUARD_MAX_CHUNKS (10),
...
def judge_target(source: dict, acquire) -> tuple[str, str]:
    ...
    if blocked:
        return "block", f"{source} -> {reason}"
    return "clean", f"{source} clean ({summary})"
